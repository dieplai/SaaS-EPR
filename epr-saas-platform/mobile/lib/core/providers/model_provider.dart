...
// Simplified model system for EPR Legal single model
// Backend: ai-chatbot-service:8004
...
// ========== EPR LEGAL PROVIDER ==========
// Custom provider for EPR Legal chatbot service
class EPRProvider extends BaseProvider {
  @override
  Future<List<ModelInfo>> listModels(ProviderConfig cfg) async {
    // EPR Legal always provides a single model "epr-legal-ai"
    return [
      ModelRegistry.infer(
        ModelInfo(
          id: 'epr-legal-ai',
          displayName: 'EPR Legal AI',
          type: ModelType.chat,
...
class ProviderManager {
  ...
  static BaseProvider forConfig(ProviderConfig cfg) {
    ...
    if (cfg.id.toLowerCase().contains('epr')) {
      return EPRProvider();
    }
