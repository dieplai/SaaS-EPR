...
// Middleware này verify JWT token và set user info vào context
...
// Authenticate verifies JWT token and sets user info in context
// Middleware này chạy TRƯỚC handler để verify token
...
// OptionalAuthenticate attempts to authenticate but doesn't fail if token is missing
// Middleware này cho phép request đi qua dù không có token
// Useful cho endpoints public nhưng có thể personalize nếu có token
func (m *JWTMiddleware) OptionalAuthenticate() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Parse token (same logic as Authenticate)
		parts := strings.SplitN(authHeader, " ", 2)
		if len(parts) != 2 || parts[0] != "Bearer" {
			c.Next()
			return
		...
		if err == nil && token.Valid {
			if claims, ok := token.Claims.(*usecase.JWTClaims); ok {
				c.Set("userID", claims.UserID)
				c.Set("email", claims.Email)
			}
		}
...
// Middleware này cho phép frontend (React/Flutter) call API từ domain khác
...
// Middleware này log TẤT CẢ requests (method, path, status, latency)
...
// RateLimiter limits requests per IP/user to prevent abuse
// TODO: Implement rate limiting với Redis
// Example: 100 requests per minute per IP
func RateLimiter() gin.HandlerFunc {
	return func(c *gin.Context) {
		// TODO: Check Redis for request count
