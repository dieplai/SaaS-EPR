...
// import 'package:desktop_drop/desktop_drop.dart'; // Desktop-only, removed for Android build
...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin, RouteAware, WidgetsBindingObserver {
  ...
  // Desktop support removed for Android-only build
  Widget _wrapWithDropTarget(Widget child) {
    // if (!_isDesktopPlatform) return child;
    return child; // Always return child - Android only build
    /* Removed desktop DropTarget code
    return DropTarget(
      onDragEntered: (_) {
