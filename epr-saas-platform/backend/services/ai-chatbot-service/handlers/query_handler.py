...
from systems.reranker import Reranker
...
class QueryHandler:
    def __init__(self, faq_system, pdf_catalog_system, app_info_system, retriever, query_engine,
        ...
        # Initialize reranker for better retrieval accuracy
        try:
            self.reranker = Reranker()
            logger.info("Reranker system initialized")
    ...
    def process_query(self, query_text: str, session_id: Optional[str] = None) -> Dict:
        ...
        if self.scope_checker:
            ...
            if self.scope_checker.is_greeting_or_chitchat(query_text):
                ...
                # If there's conversation context, assume follow-up questions are on-topic
                # This handles cases like "chi phí là bao nhiêu?" following EPR discussion
                if conversation_context and len(conversation_context) > 2:
                    logger.info("Conversation context exists - treating as on-topic follow-up")
                    is_on_topic = True
                    scope_info = {
                        'is_on_topic': True,
                        'confidence': 'high',
                        'reason': 'Follow-up question in ongoing conversation'
                    }
                else:
                    # Use LLM to check scope only if no conversation context
    ...
    def _process_legal_query(
        ...
        # Rerank nodes for better relevance
        if nodes and self.reranker:
            nodes = self.reranker.rerank_nodes(query_text, nodes, top_k=Config.MAX_SOURCES * 2)
            logger.info(f"Reranked {len(nodes)} nodes")
    ...
    def _query_with_context(
        ...
        try:
            ...
            if nodes and self.reranker:
                nodes = self.reranker.rerank_nodes(query_text, nodes, top_k=8)
            ...
            response = self.openai_client.chat.completions.create(
                ...
                temperature=0.2,  # Increased for more natural responses
                max_tokens=2500  # Increased for comprehensive business-oriented analysis
            )
    ...
    def _query_with_fallback(
        ...
        if hasattr(self.retriever, '_index'):
            ...
            enhanced_llm = OpenAI(model=Config.LLM_MODEL, temperature=0.1)
