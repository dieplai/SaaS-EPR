...
import 'package:epr_legal_mobile/secrets/fallback.dart';

// ========== EPR LEGAL INTEGRATION ==========
// Backend URL: http://localhost:8004/api/v1/query (AI Chatbot Service)
// Authentication: Bearer token from user-service (port 8001)
// Note: EPR backend doesn't support streaming; returns full JSON response
...
class ChatApiService {
  ...
  static Stream<ChatStreamChunk> sendMessageStream({
    ...
    try {
      // Check if this is EPR Legal provider
      if (config.id.toLowerCase().contains('epr')) {
        yield* _sendEPRStream(
          client,
          config,
          modelId,
          messages,
  ...
  // ========== EPR LEGAL STREAM IMPLEMENTATION ==========
  static Stream<ChatStreamChunk> _sendEPRStream(
    http.Client client,
    ProviderConfig config,
    String modelId,
    List<Map<String, dynamic>> messages,
    {double? temperature, double? topP, int? maxTokens, Map<String, String>? extraHeaders, Map<String, dynamic>? extraBody}
  ) async* {
    final baseUrl = config.baseUrl.endsWith('/')
        ? config.baseUrl.substring(0, config.baseUrl.length - 1)
        : config.baseUrl;
    ...
    // Prepare request body for EPR endpoint
    final body = {
      'query': query,
    ...
    try {
      ...
      try {
        ...
        final answer = data['answer'] ?? 'No response from server';
        final numSources = data['num_sources'] ?? 0;
        final sources = data['sources'] ?? [];

        // Yield the complete response as a single chunk
        // (EPR API doesn't support streaming, so we return the full response at once)
        yield ChatStreamChunk(
          content: answer,
          isDone: true,
          totalTokens: 0,
          // Optional: could add metadata about sources
          // metadata: {'num_sources': numSources, 'sources': sources}
        );
      } catch (e) {
        yield ChatStreamChunk(
          content: 'Error parsing response: ${e.toString()}',
          isDone: true,
          totalTokens: 0,
        );
      }
